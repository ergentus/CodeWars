...
export function getSum(number: number): number {
	return String(number).split('').reduce((a, e) => a + +e, 0)
}
...
export const isEvenIndexSumGreater = (arr1: Array<number>): boolean => {
	let evenArr = 0
	let oddArr = 0
...
export function getSquarePositiveIntegers(array: Array<number>): Array<number> {
	return [...array].filter(e => Number.isInteger(e) && e > 0).map(e => e ** 2)
}
